...
def seed_db(db: Session = Depends(get_db)):
    ...
    default_nursery = db.query(models.Nursery).filter(models.Nursery.username == "saplinggo_official").first()
    if not default_nursery:
        default_nursery = models.Nursery(
            nursery_name="Saplinggo Official Nursery",
            username="saplinggo_official",
            email="nursery@saplinggo.com",
            phone_number="1234567890",
            password="password123"
        )
        db.add(default_nursery)
        db.commit()
        db.refresh(default_nursery)
    else:
        db.commit()
