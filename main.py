...
@app.put("/api/plants/{plant_id}/restock", response_model=schemas.Plant)
def restock_plant(plant_id: int, req: RestockRequest, db: Session = Depends(get_db)):
    plant = db.query(models.Plant).filter(models.Plant.id == plant_id).first()
    if not plant:
        raise HTTPException(status_code=404, detail="Plant not found")
...
@app.get("/api/nursery/{nursery_id}/stats")
def get_nursery_stats(nursery_id: int, db: Session = Depends(get_db)):
    plants = db.query(models.Plant).filter(models.Plant.nursery_id == nursery_id).all()
    plants_present = sum([p.quantity for p in plants])
    
    # Calculate sold plants and earnings (only for booked orders)
    order_items = db.query(models.OrderItem).join(models.Order).join(models.Plant).filter(
        models.Plant.nursery_id == nursery_id,
        models.Order.status == 'booked'
    ).all()
    plants_sold = sum([item.quantity for item in order_items])
    total_earned = sum([item.quantity * item.price for item in order_items])
...
@app.get("/api/individual-sellers")
def get_individual_sellers(db: Session = Depends(get_db)):
    sellers = db.query(models.User).join(models.Plant, models.Plant.user_id == models.User.id).distinct().all()
    return [{"id": s.id, "name": s.name, "email": s.email, "phone_number": s.phone_number} for s in sellers]
...
@app.post("/api/nursery/register", response_model=schemas.Nursery)
def register_nursery(nursery: schemas.NurseryCreate, db: Session = Depends(get_db)):
    db_nursery = db.query(models.Nursery).filter(
        or_(
            func.lower(models.Nursery.username) == nursery.username.lower(), 
            func.lower(models.Nursery.email) == nursery.email.lower()
        )
    ).first()
    if db_nursery:
        raise HTTPException(status_code=400, detail="Username or email already registered")
...
@app.post("/api/nursery/login", response_model=schemas.Nursery)
def login_nursery(nursery: schemas.NurseryLogin, db: Session = Depends(get_db)):
    db_nursery = db.query(models.Nursery).filter(
        or_(
            func.lower(models.Nursery.username) == nursery.username.lower(), 
            func.lower(models.Nursery.email) == nursery.username.lower()
        ),
        models.Nursery.password == nursery.password
    ).first()
    if not db_nursery:
        raise HTTPException(status_code=400, detail="Invalid credentials")
    return db_nursery
...
@app.post("/api/checkout/{user_id}", response_model=List[schemas.OrderResponse])
def checkout_cart(user_id: int, request: schemas.CheckoutRequest, db: Session = Depends(get_db)):
    ...
    for item in cart_items:
        key = (item.plant.nursery_id, item.plant.user_id)
...
@app.get("/api/nurseries", response_model=List[schemas.Nursery])
def read_nurseries(db: Session = Depends(get_db)):
...
def seed_db(db: Session = Depends(get_db)):
    ...
    # Create 5 Sample Nurseries
    nursery_data = [
        {"nursery_name": "Saplinggo Official Nursery", "username": "saplinggo_official", "email": "official@saplinggo.com", "phone_number": "1234567890", "password": "password123"},
        {"nursery_name": "Urban Roots", "username": "urban_roots", "email": "urban@saplinggo.com", "phone_number": "2345678901", "password": "password123"},
        {"nursery_name": "Nature's Palette", "username": "natures_palette", "email": "nature@saplinggo.com", "phone_number": "3456789012", "password": "password123"},
        {"nursery_name": "The Sprout Studio", "username": "sprout_studio", "email": "sprout@saplinggo.com", "phone_number": "4567890123", "password": "password123"},
        {"nursery_name": "Oasis Flora", "username": "oasis_flora", "email": "oasis@saplinggo.com", "phone_number": "5678901234", "password": "password123"}
    ]
    
    nursery_ids = []
    for data in nursery_data:
        nursery = db.query(models.Nursery).filter(models.Nursery.username == data["username"]).first()
        if not nursery:
            nursery = models.Nursery(**data)
            db.add(nursery)
            db.commit()
            db.refresh(nursery)
        nursery_ids.append(nursery.id)
...
@app.get("/api/nursery/{nursery_id}/pickups", response_model=List[schemas.OrderResponse])
def get_nursery_pickups(nursery_id: int, db: Session = Depends(get_db)):
    orders = db.query(models.Order).filter(models.Order.nursery_id == nursery_id).order_by(models.Order.created_at.desc()).all()
    return orders
